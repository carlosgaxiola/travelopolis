...
CREATE TABLE `detalle_viajes` (
  ...
  `status` int(1) DEFAULT NULL COMMENT '0= Cancelado/ 1= Datos Enviados / 2=Dio Anticipo / 3 = Liquidó',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=latin1;

/*Data for the table `detalle_viajes` */

insert  into `detalle_viajes`(`id`,`id_viaje`,`id_viajero`,`cantidad`,`resto`,`f_registro`,`status`) values (1,1,22,1,2400,'2019-03-27',1),(2,1,21,1,2400,'2019-03-27',1),(3,3,22,1,0,'2019-03-28',0),(4,1,1,1,2400,'2019-03-30',1),(5,2,22,1,6500,'2019-04-01',0),(6,2,21,1,6500,'2019-04-01',0),(7,1,26,3,7200,'2019-04-02',1),(9,2,26,1,6500,'2019-04-02',1);
...
insert  into `perfiles_modulos`(`id`,`id_perfil`,`id_modulo`,`f_registro`,`status`) values (10,1,2,NULL,NULL),(11,1,3,NULL,NULL),(12,1,4,NULL,NULL),(13,1,5,NULL,NULL),(14,1,6,NULL,NULL);
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`max`@`%` SQL SECURITY DEFINER VIEW `listar_detalle_viaje` AS (select `cli`.`id` AS `id`,`cli`.`nombre` AS `nombre`,`cli`.`a_paterno` AS `a_paterno`,`cli`.`a_materno` AS `a_materno`,`cli`.`sexo` AS `sexo`,`cli`.`edad` AS `edad`,`cli`.`estado` AS `estado`,`cli`.`telefono` AS `telefono`,`cli`.`correo` AS `correo`,`cli`.`informacion` AS `informacion`,`cli`.`id_usuario` AS `id_usuario`,`cli`.`f_registro` AS `f_registro`,`cli`.`status` AS `status`,`det`.`cantidad` AS `cantidad`,`det`.`resto` AS `resto`,`via`.`nombre` AS `viaje` from ((`viajeros` `cli` join `detalle_viajes` `det` on((`cli`.`id` = `det`.`id_viajero`))) join `viajes` `via` on((`via`.`id` = `det`.`id_viaje`)))) */;
